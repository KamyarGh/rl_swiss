...
def experiment(exp_specs):
    ...
    fc_decoder = nn.Sequential(
        nn.Linear(gru_dim + act_dim, flat_inter_img_dim, bias=False),
    ...
    for iter_num in range(int(float(exp_specs['max_iters']))):
        ...
        hidden = fc_decoder(torch.cat([prev_h_batch, act_batch], 1)).view(obs_batch.size(0), conv_channels, img_h, img_h)
        ...
        if iter_num % (500*episode_length) in range(2*episode_length):
            save_pytorch_tensor_as_img(recon[0].data.cpu(), 'junk_vis/pogrid_4_len_8_scale_4/rnn_recon_%d.png' % iter_num)
            save_pytorch_tensor_as_img(obs_batch[0].data.cpu(), 'junk_vis/pogrid_4_len_8_scale_4/rnn_obs_%d.png' % iter_num)
        ...
        if iter_num % exp_specs['freq_val'] == 0:
            ...
            for i in range(episode_length):
                ...
                hidden = fc_decoder(torch.cat([val_prev_h_batch, act_batch], 1)).view(obs_batch.size(0), conv_channels, img_h, img_h)
                hidden = conv_decoder(hidden)
                recon = mean_decoder(hidden)
                log_cov = log_cov_decoder(hidden)
                log_cov = torch.clamp(log_cov, LOG_COV_MIN, LOG_COV_MAX)
