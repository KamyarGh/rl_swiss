...
def experiment(exp_specs):
    ...
    for iter_num in range(int(float(exp_specs['max_iters']))):
        ...
        if iter_num % (500*episode_length) in range(2*episode_length):
            save_pytorch_tensor_as_img(recon[0].data.cpu(), 'junk_vis/onthefly_conv_gru_pogrid_len_8_scale_4/rnn_recon_%d.png' % iter_num)
            save_pytorch_tensor_as_img(obs_batch[0].data.cpu(), 'junk_vis/onthefly_conv_gru_pogrid_len_8_scale_4/rnn_obs_%d.png' % iter_num)
