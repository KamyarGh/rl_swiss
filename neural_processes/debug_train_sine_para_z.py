...
class BaseMap(nn.Module):
    def __init__(self):
        ...
        dim = 160
        z_dim = 40
        z_mod_list = nn.ModuleList()
        mod_list = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Linear(z_dim + 1, dim),
                    nn.BatchNorm1d(dim),
                    nn.ReLU(),
                )
            ]
        ...
        self.z_mod_list = z_mod_list
        self.last = nn.Linear(dim, 1)
    
    def forward(self, z, x):
        out = x
        z_cur = z
        for z_mod, mod in zip(self.z_mod_list, self.mod_list):
            out = mod(torch.cat([z_cur,out], 1))
            z_cur = z_mod(z_cur)
        out = self.last(out)
        return out
