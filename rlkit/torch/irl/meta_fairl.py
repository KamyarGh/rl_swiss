...
class MetaFAIRL(TorchMetaIRLAlgorithm):
    def __init__(
            self,
            env,
            policy,
    ...
    def _get_disc_training_batch(self):
        ...
        policy_test_pred_batch = {
            ...
            'next_observations': np.concatenate((policy_test_pred_batch_0['next_observations'], policy_test_pred_batch_1['next_observations']), axis=0)
        }
    ...
    def _do_reward_training(self, epoch, update_loop_iter, disc_update_iter):
        ...
        if self.transfer_version:
            ...
            # disc_logits = self.discriminator(obs_batch, acts_batch, next_obs_batch, repeated_z_batch)
            disc_logits = self.discriminator(obs_batch, acts_batch, next_obs_batch, repeated_z_batch)
        ...
        if self.use_grad_pen:
            ...
            if self.state_only:
                gradients = autograd.grad(
                    outputs=self.discriminator(interp_obs, None, interp_next_obs, z_batch.detach()).sum(),
                    inputs=[interp_obs, interp_next_obs],
                ...
                if self.transfer_version:
                    ...
                    gradients = autograd.grad(
                        outputs=self.discriminator(interp_obs, interp_actions, interp_next_obs, z_batch.detach()).sum(),
                        inputs=[interp_obs, interp_actions, interp_next_obs],
                        # grad_outputs=torch.ones(exp_specs['batch_size'], 1).cuda(),
                        create_graph=True, retain_graph=True, only_inputs=True
                    )
                    total_grad = torch.cat([gradients[0], gradients[1], gradients[2]], dim=1)
        ...
        if self.eval_statistics is None:
            ...
            if self.use_target_disc:
                if self.state_only:
                    target_disc_logits = self.target_disc(obs_batch, None, next_obs_batch, repeated_z_batch)
                else:
                    if self.transfer_version:
                        raise NotImplementedError()
                        ...
                        target_disc_logits = self.target_disc(obs_batch, acts_batch, next_obs_batch, repeated_z_batch)
                ...
                if self.use_grad_pen:
                    ...
                    if self.state_only:
                        target_gradients = autograd.grad(
                            outputs=self.target_disc(interp_obs, None, interp_next_obs, z_batch).sum(),
                            inputs=[interp_obs, interp_next_obs],
                            # grad_outputs=torch.ones(exp_specs['batch_size'], 1).cuda(),
                            create_graph=True, retain_graph=True, only_inputs=True
                        )
                        ...
                        if self.transfer_version:
                            ...
                            interp_actions.requires_grad = True
                            target_gradients = autograd.grad(
                                outputs=self.target_disc(interp_obs, interp_actions, interp_next_obs, z_batch).sum(),
                                inputs=[interp_obs, interp_actions, interp_next_obs],
                                # grad_outputs=torch.ones(exp_specs['batch_size'], 1).cuda(),
                                create_graph=True, retain_graph=True, only_inputs=True
                            )
                            total_target_grad = torch.cat([target_gradients[0], target_gradients[1], target_gradients[2]], dim=1)
    ...
    def _do_policy_training(self, epoch):
        ...
        disc_for_rew.eval()
        if self.state_only:
            policy_batch['rewards'] = disc_for_rew(policy_batch['observations'], None, policy_batch['next_observations'], z).detach()
        else:
            if self.transfer_version:
                raise NotImplementedError()
                # compute the log probability of actions under the policy
                ...
                policy_batch['rewards'] = disc_for_rew(policy_batch['observations'], policy_batch['actions'], policy_batch['next_observations'], z).detach()
        disc_for_rew.train()
