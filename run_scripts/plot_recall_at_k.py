...
def plot_figure(few_shot, prior, label, save_path):
    ...
    ax.errorbar(x_axis, prior['mean'], prior['std'],
        elinewidth=2.0, capsize=4.0, barsabove=True, linewidth=2.0, label='Prior',
        color='magenta'
    )
    # ax.plot(x_axis, [prior['mean'][0] for _ in range(8)], label='Prior', linestyle='--', color='magenta')
    ax.set_ylim([0.0, 1.0])
    ax.set_xlim([0.75, 8.25])
    ax.set_xlabel('Number of Posterior Samples')
...
np_bc_few_shot = joblib.load(osp.join(np_bc_path, 'formatted_recall_at_k.pkl'))['solved']
prior_np_bc_few_shot = joblib.load(osp.join(np_bc_path, 'prior_sampled_formatted_recall_at_k.pkl'))['solved']

np_airl_few_shot = joblib.load(osp.join(np_airl_path, 'formatted_recall_at_k.pkl'))['solved']
prior_np_airl_few_shot = joblib.load(osp.join(np_airl_path, 'prior_sampled_formatted_recall_at_k.pkl'))['solved']

plot_figure(
    np_bc_few_shot,
    prior_np_bc_few_shot,
    'Meta-BC',
    'plots/final_plots/npbcrecall.png'
)
plot_figure(
    np_airl_few_shot,
    prior_np_airl_few_shot,
    'Meta-AIRL',
    'plots/final_plots/npairlrecall.png'
)
